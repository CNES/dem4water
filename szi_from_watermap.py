...
def main(arguments):
    ...
    parser.add_argument("-w", "--watermap", required=True, help="Input water map file")
    parser.add_argument("-d", "--dem", required=True, help="Input DEM")
    parser.add_argument("--zmin", type=float, required=True, help="Bottom altitude")
    parser.add_argument("--zmax", type=float, required=True, help="Starting altitude")
    parser.add_argument("-t", "--tmp", required=True, help="Temporary directory")
    parser.add_argument(
        "-s", "--step", type=int, default=5, help="Altitude sampling step"
    )
    ...
    pixelSizeY = -gt[5]
    logging.debug(
        "pixelSizeX: " + str(pixelSizeX) + " - pixelSizeY: " + str(pixelSizeY)
    )
    ...
    msk.SetParameterString(
        "out", os.path.join(args.tmp, "mask_@" + str(args.zmax) + "m.tif")
    )
    msk.SetParameterOutputImagePixelType("out", otb.ImagePixelType_uint8)
    msk.SetParameterString(
        "exp", "( im1b1  > 0.05 ) ? ((im2b1 < " + str(args.zmax) + ") ? 1 : 0 ) : 0"
    )
    #  msk.SetParameterString("exp", "( im1b1  > 0.10 ) ? ((im2b1 < "+str(args.zmax)+") ? 1 : 0 ) : 0")
    #  msk.SetParameterString("exp", "( im1b1  > 0.10 ) ? 1 : 0 ")
    if (
        os.path.exists(os.path.join(args.tmp, "mask_@" + str(args.zmax) + "m.tif"))
        is False
    ):
    ...
    mdil.SetParameterString(
        "in", os.path.join(args.tmp, "mask_@" + str(args.zmax) + "m.tif")
    )
    mdil.SetParameterString(
        "out", os.path.join(args.tmp, "maskdil_@" + str(args.zmax) + "m.tif")
    )
    ...
    mbm.SetParameterString(
        "out", os.path.join(args.tmp, "lbmap_@" + str(args.zmax) + "m.tif")
    )
    mbm.SetParameterString(
        "exp", "( (im1b1 < " + str(args.zmax) + ") ? im1b1 * im2b1 : 0 )"
    )
    mbm.Execute()
    ...
    seg.SetParameterString(
        "mode.raster.out",
        os.path.join(args.tmp, "tmplbmap_@" + str(args.zmax) + "m.tif"),
    )
    ...
    if (
        os.path.exists(os.path.join(args.tmp, "tmplbmap_@" + str(args.zmax) + "m.tif"))
        is False
    ):
    ...
    for lab in range(2, int(max_label + 1)):
        results_surf = np.count_nonzero((np_surf == [lab]))
        if results_surf > best_surf:
            logging.debug(
                "new best_surf: " + str(results_surf) + " [label: " + str(lab) + "]"
            )
            best_label = lab
    ...
    kp.AddImageToParameterInputImageList(
        "il", wshed.GetParameterOutputImage("mode.raster.out")
    )
    ...
    kp.SetParameterString(
        "out", os.path.join(args.tmp, "kp_@" + str(args.zmax) + "m.tif")
    )
    ...
    for i in range(math.ceil(args.zmax), math.floor(args.zmin), -1 * args.step):
        ...
        msk = otb.Registry.CreateApplication("BandMath")
        #  msk.SetParameterStringList("il", [args.watermap, args.dem])
        msk.SetParameterStringList(
            "il", [args.dem, os.path.join(args.tmp, "kp_@" + str(args.zmax) + "m.tif")]
        )
        #  msk.SetParameterStringList("il", [args.dem])
        #  msk.AddImageToParameterInputImageList("il",kp.GetParameterOutputImage("out"));
        msk.SetParameterString(
            "out", os.path.join(args.tmp, "mask_S_Z-" + str(i) + ".tif")
        )
        msk.SetParameterOutputImagePixelType("out", otb.ImagePixelType_uint8)
    ...
    for i in range(
        math.ceil(args.zmax), math.floor(args.zmax + 10 * args.step), args.step
    ):
        ...
        if init is True:
            adil.SetParameterString(
                "in", os.path.join(args.tmp, "mask_S_Z-" + str(prev_alt) + ".tif")
            )
        else:
            adil.SetParameterString(
                "in", os.path.join(args.tmp, "up_mask_S_Z-" + str(prev_alt) + ".tif")
            )

        adil.SetParameterString(
            "out", os.path.join(args.tmp, "up_maskdil_S_Z-" + str(i) + ".tif")
        )
        ...
        amsk.SetParameterString(
            "out", os.path.join(args.tmp, "up_masktmp_S_Z-" + str(i) + ".tif")
        )
        amsk.SetParameterOutputImagePixelType("out", otb.ImagePixelType_uint8)
        amsk.SetParameterString(
            "exp",
            "(im2b1 == 1) ? im2b1 : (((im3b1 == 1)*(im1b1 > "
            + str(prev_alt)
            + ")*(im1b1 <= "
            + str(i)
            + ") ) ? 1 : 0 ) ",
        ...
        aclo.SetParameterString(
            "in", os.path.join(args.tmp, "up_masktmp_S_Z-" + str(i) + ".tif")
        )
        aclo.SetParameterString(
            "out", os.path.join(args.tmp, "up_mask_S_Z-" + str(i) + ".tif")
        )
