...
def write_json(global_config_json):
    ...
    with open(global_config_json, encoding="utf-8") as config_in:
        ...
        for dam, id_dam in dict_all_dams.items():
            ...
            output_dam_extract_path = os.path.join(output_path, "extracts", dam_path_name)
            ...
            if szi_dat_file is None:
                szi_dat_file = os.path.join(output_dam_camp_path, f"{dam_path_name}_SZi.dat")
            ...
            if daminfo_file is None:
                daminfo_file = os.path.join(output_dam_camp_path, f"{dam_path_name}_daminfo.json")
            ...
            dict_dam["szi_to_model"] = {
                ...
                "outfile": os.path.join(output_dam_camp_path, f"{dam_path_name}_model.png"),
            ...
            if reference is not None:
                dict_dam["val_report"] = {}
            json_out_file = os.path.join(output_dam_camp_path, f"params_{dam_path_name}.json")
            with open(
                json_out_file,
                "w",
                encoding="utf-8",
