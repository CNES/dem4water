...
def main(arguments):
    ...
    v_m_Zmax = V0 + math.pow(((s_r_Zmax_m2 - S0) / alpha), 1 / beta) * (
        S0 + ((s_r_Zmax_m2 - S0) / (beta + 1.0))
    )
    ...
    #  s_r_norm = []
    for a in s:
        v_m = V0 + math.pow(((a - S0) / alpha), 1 / beta) * (
            S0 + ((a - S0) / (beta + 1.0))
        )
        ...
        v_r = ref_V0 + math.pow(((a - ref_S0) / ref_alpha), 1 / ref_beta) * (
            ref_S0 + ((a - ref_S0) / (ref_beta + 1.0))
        )
    ...
    Sz = plt.subplot(gs[0])
    Sz.plot(Sz_ref_scatter, Sz_ref_scatter, "r-")
    Sz.scatter(Sz_ref_scatter, Sz_model_scatter, label="S(z)_model = f(S(z)_reference)")
    ...
    Sz_comp.plot(z, Sz_ref_scatter, "r-", label="S(z)_reference")
    Sz_comp.plot(z, Sz_model_scatter, "g-", label="S(z)_model")
    Sz_comp.axvline(
    ...
    Vs = plt.subplot(gv[0])
    Vs.plot(Vs_ref_scatter, Vs_ref_scatter, "r-")
    Vs.scatter(Vs_ref_scatter, Vs_model_scatter, label="V(S)_model = f(V(S)_reference)")
    ...
    Vs_comp.plot(s, Vs_ref_scatter, "r-", label="V(S)_reference")
    Vs_comp.plot(s, Vs_model_scatter, "g-", label="V(S)_model")
    ...
    Tx = plt.subplot(gt[0])
    Tx.plot(Tx_ref_scatter, Tx_ref_scatter, "r-")
    Tx.scatter(
    ...
    Tx_comp.plot(s, Tx_ref_scatter, "r-", label="Vr(S)_reference")
    Tx_comp.plot(s, Tx_model_scatter, "g-", label="Vr(S)_model")
